...
class Dock:
    ...
    def _release_only(self) -> None:
        ...
        self._write_now("\x1b7\x1b[r\x1b8")
    ...
    def _undo_reservation(self) -> None:
        ...
        self._write_now(self._restore_sequence())
        self._reserved = False
    ...
    def _scroll_room_into_being(self, height: int) -> None:
        ...
        """
        self._write_now(f"\x1b[{height};1H" + "\n" * self._reserved_rows)
    ...
    def _write_now(self, text: str) -> None:
        """One whole sequence onto the terminal, and out of the buffer.

        Flushed here rather than by the caller: the sequence carries no
        newline, so a line-buffered stream would hold it, and a resize
        landing in between would put its own release out first.
        """
        self._write(text)
        self._flush()
