...
class BaseItem:
    ...
    def is_published(self):
        if (
            self.content_delivery_type
            and self.content_delivery_type == "AudioPart"
            and self._parent
        ):
        ...
        if publication_datetime is not None:
            pub_date = datetime.strptime(publication_datetime, "%Y-%m-%dT%H:%M:%SZ")
            now = datetime.utcnow()
            return now > pub_date
...
class LibraryItem(BaseItem):
    ...
    async def get_aaxc_url(
        self, quality: str = "high", license_response_groups: str | None = None
    ):
        if not self.is_published():
            raise ItemNotPublished(self.asin, self.publication_datetime)
...
class BaseList:
    def __init__(self, data: dict | list, api_client: audible.AsyncClient):
        self._client = api_client
        self._data = self._prepare_data(data)
...
class Library(BaseList):
    ...
    async def from_api_full_sync(
        cls, api_client: audible.AsyncClient, bunch_size: int = 1000, **request_params
    ) -> "Library":
...
class Catalog(BaseList):
    ...
    @classmethod
    async def from_api(cls, api_client: audible.AsyncClient, **request_params):
        if "response_groups" not in request_params:
            request_params["response_groups"] = (
                "contributors, customer_rights, media, price, product_attrs, "
                "product_desc, product_extended_attrs, product_plan_details, "
