...
class DownloadCounter:
    ...
    def count_error(self):
        self._errors += 1
        logger.debug(f"Currently encountered errors: {self.errors}")
...
@click.option(
    "--title", "-t", multiple=True, help="title of the audiobook (partial search)"
)
...
@click.option(
    "--ignore-podcasts", is_flag=True, help="Ignore a podcast if it has episodes"
)
@bunch_size_option
@pass_session
...
async def cli(session, api_client, **params):
    ...
    if aax_fallback:
        ...
        if get_aaxc:
            logger.warning(
                "--aaxc is redundant with --aax-fallback (fallback already uses aaxc)."
            )
